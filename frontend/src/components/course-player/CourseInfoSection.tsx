      ...
      <div className="flex items-start justify-between">
        ...
        <div className="text-right ml-6">
          <div className="bg-background/50 rounded-lg p-4 border border-primary/20">
            ...
            <div className="text-2xl font-bold text-primary">{Math.round((course.lessons.filter(l => l.isCompleted).length / course.lessons.length) * 100)}%</div>
            <div className="text-xs text-muted-foreground mt-1">
              {course.lessons.filter(l => l.isCompleted).length} من {course.lessons.length} مكتمل
            </div>
          </div>
        </div>
      </div>
